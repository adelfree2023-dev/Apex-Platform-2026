 ...
 * Security: Uses crypto.randomBytes for token generation
 */
...
export class SocialAuthService {
  ...
  async logout(tenantSchema: string, sessionToken: string): Promise<void> {
    this.validateTenantSchema(tenantSchema);
    await this.prisma.$executeRawUnsafe(`
  ...
  /**
   * Validate tenant schema to prevent SQL injection
   * @throws BadRequestException if schema is invalid
   */
  private validateTenantSchema(schema: string): void {
    const pattern = /^tenant_[a-z0-9_]+$/;
    if (!pattern.test(schema)) {
      throw new BadRequestException('Invalid tenant schema format');
    }
  }
