...
export class SuperAdminController {
    ...
    async getTenants() {
        try {
            const tenants = await this.superAdminService.getTenants();
            return {
                success: true,
                data: tenants,
    ...
    async getTenant(@Param('id') id: string) {
        try {
            const tenant = await this.superAdminService.getTenant(id);
            if (!tenant) {
    ...
    async getTenantStats(@Param('id') id: string) {
        try {
            const stats = await this.superAdminService.getTenantStats(id);
            return {
                success: true,
                data: stats,
            };
    ...
    async getRecentOrders(@Param('id') id: string) {
        try {
            const orders = await this.superAdminService.getRecentOrders(id);
            return {
                success: true,
                data: orders,
                count: orders.length,
    ...
    async getTopProducts(@Param('id') id: string) {
        try {
            const products = await this.superAdminService.getTopProducts(id);
            return {
                success: true,
                data: products,
    ...
    async updateTenant(
        ...
        try {
            const tenant = await this.superAdminService.updateTenant(id, body);
            return {
                success: true,
                data: tenant,
